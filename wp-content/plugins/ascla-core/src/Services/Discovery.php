...
final class Discovery
{
    public static function resource(int $id): array
    {
        ...
        $terms = wp_get_object_terms($id, 'ascla_interest', ['fields' => 'ids']);
        if (is_wp_error($terms) || !$terms) { return ['sent' => 0]; }
        $sent = 0;
        self::eachMember(static function ($user) use ($post, $terms, &$sent) {
