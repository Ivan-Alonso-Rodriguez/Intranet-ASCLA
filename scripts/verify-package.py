...
out=root/'docs/evidence/release-1.7.0'
...
with zipfile.ZipFile(root/'dist/ascla-core.zip') as archive:
    ...
    assert b'Version: 1.7.0' in archive.read('ascla-core/ascla-core.php')
    secrets=[]
    ...
    report={'date':datetime.datetime.now(datetime.timezone.utc).isoformat(),'version':'1.7.0','bytes':package.stat().st_size,'sha256':digest,'files':len(names),'crc_valid':True,'source_bytes_match':True,'forbidden_paths':[],'known_local_secrets_absent':True,'translation_catalog_included':True,'production_hashes':hashes}
