...
final class CompletionTest extends TestCase
{
    ...
    public function testVideoDurationIsDetectedAndGeneratedNoteStaysInOriginalResource(): void
    {
        $p=$this->create('resource',['interest'=>[$this->terms['interest']],'meta'=>['youtube_url'=>'https://youtu.be/abcdefghijk','resource_type'=>'Video','chatham'=>false,'transcript'=>"[00:00] Inicio del contenido.
[03:30] Cierre del contenido."]]);
        $data=Knowledge::videoMetadata($p['id']); self::assertSame(210,$data['duration_seconds']);
        ...
        self::assertSame([$this->terms['interest']],wp_get_object_terms($p['id'],'ascla_interest',['fields'=>'ids']));
    }
}
