...
async function uploadPhoto(p){await p.goto(base+'/perfil/');await p.locator('[data-upload="photo"]').setInputFiles('wp-content/plugins/ascla-core/assets/ascla-logo.png');await p.locator('.ascla-image-editor [data-image-apply]').waitFor();await p.locator('.ascla-image-editor [data-image-apply]').click();await until(async()=>Number(await p.locator('[name="photo_id"]').inputValue())>0,'Photo upload did not finish');const id=Number(await p.locator('[name="photo_id"]').inputValue());await ok(p,'profiles/me',{photo_id:id});return id;}
(async()=>{
