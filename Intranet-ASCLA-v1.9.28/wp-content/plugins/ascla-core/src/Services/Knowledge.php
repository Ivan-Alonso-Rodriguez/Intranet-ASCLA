...
final class Knowledge
{
    ...
    private static function syncVideoMetadata(int $id,bool $strict): array
    {
        ...
        if ($duration<=0) {
            try {
                ...
                if ($strict && $previousDuration<=0) { throw ($apiError??$e); }
            }
        }
    ...
    public static function multimedia(int $id,?AIProviderInterface $ai=null): array
    {
        ...
        return ['resource_id'=>$id,'capsule_ids'=>[],'hub_id'=>0,'mode'=>$ai->mode(),'video_mode'=>$videoMode,'duration_seconds'=>$durationSeconds,'message'=>'Resumen y nota técnica integrados en la publicación original.'];
    }
