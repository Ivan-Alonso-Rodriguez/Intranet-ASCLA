...
final class YouTubeVideoProvider implements VideoProviderInterface
{
    ...
    public static function publicMetadata(string $videoId): array
    {
        ...
        $url='https://www.youtube.com/watch?v='.rawurlencode($videoId).'&hl=es';
        $response=wp_safe_remote_get($url,['timeout'=>6,'redirection'=>2,'limit_response_size'=>4194304,'headers'=>['User-Agent'=>'Mozilla/5.0 (compatible; ASCLA/'.(defined('ASCLA_VERSION')?ASCLA_VERSION:'1').')','Accept-Language'=>'es,en;q=0.8']]);
        if (is_wp_error($response) || wp_remote_retrieve_response_code($response)!==200) { throw new \RuntimeException('No fue posible consultar la duración pública del video.'); }
        $body=wp_remote_retrieve_body($response); $duration=0;
