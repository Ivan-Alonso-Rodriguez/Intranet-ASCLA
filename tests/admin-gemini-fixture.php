...
if(($in['action']??'')==='setup') {
    ...
    add_option($option,['settings'=>get_option('ascla_settings',false),'key'=>get_option('ascla_secret_ai_key',false),'openai_key'=>get_option('ascla_secret_openai_key',false)],'','no');
    $users=[];
    ...
    delete_option('ascla_secret_ai_key');delete_option('ascla_secret_openai_key');$settings=ASCLA\Core\Services\Settings::get();$settings['ai_mode']='mock';$settings['ai_provider']='mock';$settings['ai_model']='';$settings['openai_model']='gpt-5.6-luna';update_option('ascla_settings',$settings,false);
    ...
    foreach(['settings'=>'ascla_settings','key'=>'ascla_secret_ai_key','openai_key'=>'ascla_secret_openai_key'] as $key=>$name){if($old[$key]===false)delete_option($name);else update_option($name,$old[$key],false);}
