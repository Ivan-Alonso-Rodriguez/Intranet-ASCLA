...
final class ContentLifecycleTest extends TestCase
{
    ...
    public function testFreshMultimediaDemoKeepsAdminContextAndMembersCannotSeed():void
    {
        ...
        try {
            ...
            $result=get_option('ascla_demo_showcase');foreach(array_merge([$result['resource_id'],$result['hub_id']],$result['capsule_ids']) as $id)$this->posts[]=$id;
            self::assertSame($this->users[0],get_current_user_id());self::assertSame($this->users[0],(int)get_post($source)->post_author);self::assertSame('draft',get_post_status($result['resource_id']));
