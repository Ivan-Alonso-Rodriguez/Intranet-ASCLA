...
final class SprintNetworkingTest extends TestCase
{
    ...
    public function testMockMultimediaProducesGroundedStructuredDraftsAndUsableCapsules(): void {
        ...
        update_post_meta($id,'_ascla',['transcript'=>DemoShowcase::transcript(),'chatham'=>true,'video_id'=>'M7lc1UVf-VE']);$r=Knowledge::multimedia($id);$this->posts=array_merge($this->posts,[$r['resource_id'],$r['hub_id']],$r['capsule_ids']);$meta=get_post_meta($r['resource_id'],'_ascla',true);
        self::assertSame('draft',get_post_status($r['resource_id']));self::assertNotEmpty($meta['frameworks'],'frameworks');self::assertNotEmpty($meta['norms'],'norms');self::assertNotEmpty($meta['infographic']['statistics'],'statistics');self::assertNotEmpty($meta['infographic']['timeline'],'timeline');
        $text=wp_json_encode($meta);foreach(['Juan Pérez','Compañía Boreal'] as $identity)self::assertStringNotContainsString($identity,$text);self::assertNotEmpty($r['capsule_ids']);
        foreach($r['capsule_ids'] as $clipId){$clip=get_post_meta($clipId,'_ascla',true)['clip'];self::assertGreaterThanOrEqual(60,$clip['duration']);self::assertLessThanOrEqual(180,$clip['duration']);self::assertSame($id,$clip['source_id']);self::assertStringContainsString('&t=',$clip['youtube_url']);self::assertNotEmpty($clip['reason']);}
    }
    public function testNaturalGeminiOutputPreservesParaphrasesButRedactsIdentitiesAndRechecksSourceAccess(): void {
        ...
        $result=Knowledge::multimedia($id);$this->posts=array_merge($this->posts,[$result['resource_id'],$result['hub_id']],$result['capsule_ids']);
        $meta=get_post_meta($result['resource_id'],'_ascla',true);$text=wp_json_encode($meta,JSON_UNESCAPED_UNICODE).' '.get_post($result['resource_id'])->post_content;
        foreach(['María','Aurora','Juan Pérez','Boreal'] as $unsupported)self::assertStringNotContainsString($unsupported,$text);
        self::assertStringContainsString('35 %',$text);self::assertSame('draft',get_post_status($result['resource_id']));self::assertSame('draft',get_post_status($result['hub_id']));
