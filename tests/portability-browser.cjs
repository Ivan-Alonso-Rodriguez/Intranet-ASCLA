...
(async()=>{
  ...
  try {
    ...
    const demoProfile=await browser.newPage();await demoProfile.goto('http://localhost:8089/wp-login.php');await demoProfile.locator('#user_login').fill('demo.prueba@example.invalid');await demoProfile.locator('#user_pass').fill(env.ASCLA_PROFILE_DEMO_PASSWORD);await Promise.all([demoProfile.waitForNavigation(),demoProfile.locator('#wp-submit').click()]);await demoProfile.locator('.hero').waitFor();assert.match(await demoProfile.locator('.header-profile strong').innerText(),/Usuario Demo/);await demoProfile.close();
