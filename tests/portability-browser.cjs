...
(async()=>{
  ...
  try {
    ...
    assert.equal(checked.length,12);assert.deepEqual(errors,[]);
    ...
    fs.writeFileSync(path.join(root,'test-results/portability-browser.json'),JSON.stringify({date:new Date().toISOString(),environment:'Fresh WordPress 6.8.3, ZIP only, default query-string permalinks',pages:checked,jsErrors:errors,passed:true},null,2));
    coverage.push(...await page.coverage.stopJSCoverage());fs.writeFileSync(path.join(root,'coverage/zip-v8.json'),JSON.stringify(coverage.filter(c=>c.url.includes('/ascla-core/assets/app.js'))));
