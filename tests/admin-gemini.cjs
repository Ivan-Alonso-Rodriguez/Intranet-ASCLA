...
(async()=>{let data,browser;try{
 ...
 await test('Configuración Gemini: campos, guardar modelo, eliminar clave y prueba de error segura',async()=>{
  await a.locator('[data-action="admin-tab"][data-tab="configuracion"]').click();await a.locator('[name="ai_provider"] option[value="gemini"]').waitFor({state:'attached'});assert.equal(await a.locator('[name="ai_provider"] option[value="gemini"]').innerText(),'Google Gemini · API real');assert.equal(await a.locator('[name="ai_provider"] option[value="openai"]').innerText(),'OpenAI / ChatGPT · API real');assert.equal(await a.locator('[name="ai_provider"] option[value="deepseek"]').innerText(),'DeepSeek · API real');
