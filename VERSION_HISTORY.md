...
- **Mantenibilidad / SonarQube:** el soporte común de prompts, JSON, autenticación Bearer y pruebas de conexión se extrae de los proveedores OpenAI/DeepSeek para eliminar bloques duplicados. Las migraciones repetitivas de columnas se consolidan en `MigrationSchema` y una secuencia declarativa para los esquemas 4–10. Se refactorizan además los puntos de complejidad, ternarios anidados, retornos excesivos y APIs obsoletas detectados en `app.js`, `GoogleOAuth.php` y `Events.php`, conservando el comportamiento funcional.
